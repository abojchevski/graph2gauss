...
def train_val_test_split_adjacency(A, p_val=0.10, p_test=0.05, seed=0, neg_mul=1,
    ...
    if every_node:
        if connected:
            ...
            # use (approximate) minimum edge cover to find the minimum set of edges that cover every node
            ...
            d_nic = A.sum(1).A1[not_in_cover]  # degree of nodes not in the cover
    ...
    # generate slightly more than needed completely random non-edge indices and discard any that hit an edge
    ...
    test_zeros = random_sample[:n_test]
